...
export function useWebRTCMeeting({
  ...
  /** Tracks which peer connections were initiated as an offerer by us */
      ...
      pc.oniceconnectionstatechange = () => {
        ...
        if (pc.iceConnectionState === "failed") {
          ...
          try {
            ...
            const isInitiator = initiatorsRef.current.has(remoteSocketId);
            if (isInitiator) {
              pc.createOffer({ iceRestart: true })
                ...
                .catch((err) => {
                  debug("ice-restart-offer-failed", { remoteSocketId, error: String(err) });
                });
            }
          } catch (e) {
            debug("ice-restart-failed", { remoteSocketId, error: String(e) });
          }
        }
      };
  ...
  const handleUserJoined = useCallback(
    async (payload: SignalUserJoined): Promise<void> => {
      ...
      if (peerConnsRef.current.has(remoteSocketId)) {
        ...
        initiatorsRef.current.delete(remoteSocketId);
        debug("stale-peer-connection-closed", { remoteSocketId });
      }

      const pc = createPeerConnection(remoteSocketId);
      initiatorsRef.current.add(remoteSocketId);
  ...
  const handleOffer = useCallback(
    async (payload: SignalOfferReceived): Promise<void> => {
      ...
      if (peerConnsRef.current.has(fromSocketId)) {
        ...
        initiatorsRef.current.delete(fromSocketId);
        iceCandidateQueuesRef.current.set(fromSocketId, []);
        debug("stale-peer-connection-closed-before-answer", { fromSocketId });
      }

      initiatorsRef.current.delete(fromSocketId); // We are the answerer, so we did not initiate
